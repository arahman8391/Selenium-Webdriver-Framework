...
public class BaseTest {
	...
	@BeforeTest
	public void OpenBrowserAndNavigateToLandingPage() 
	{
		try { 
			String Browser = ExcelDriver.getCelldata(1,0,TEST_ENV_SHEET_NAME, FILE_PATH);
			String URL = ExcelDriver.getCelldata(1,1,TEST_ENV_SHEET_NAME, FILE_PATH);
			
			driver = BrowserFactory.StartBrowser(Browser, URL);
			All_Script_Okta_Login_Page_Steps o = new All_Script_Okta_Login_Page_Steps(driver);
			o.Login_To_All_Script_Okta_Page(Clinical_Staff);
			log.info("Successfully Opened " + Browser + " Browser and Navigated to " + driver.getCurrentUrl());
			o.SelectTouchWorksDevPOC();
			BaseTest.WaitUntilClickable(driver, By.xpath("//*[@id=\"shellPage\"]"), 120);
			
			if(driver.findElement(By.xpath("//*[@id=\"shellPage\"]")).isDisplayed()) {
				log.info("Successfully Navigated to " + driver.getCurrentUrl());
			}
			else {
				log.error("Unable To Start Browser & Naivgate TouchWorks Home Page....");
			}
		} 
		catch (IOException e) {
			e.printStackTrace();
	...
	@AfterMethod
	public void CaputeFailedScreenshots(ITestResult iTestResult) 
	{
		try {
			checkFailedStatus(iTestResult);
			log.info("Successfully Checked For Failed Testcases....");
	...
	private void deleteFailedScreenshots(ITestContext iTestContext) 
	{
		try {
			...
			e.printStackTrace();
			log.error("Unable To Delete Failed Screenshots....");
		}
	}
	...
	private void deletePassedScreenshots(ITestContext iTestContext) 
	{
		try {
			...
			e.printStackTrace();
			log.error("Unable To Delete Passed Screenshots....");
		}
	}
...
//    		ImageIO.write(screenshot.getImage(), "PNG", new File("src/main/resources/passed-screenshots/"+ fileName +".PNG"));
    		...
    		File src = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
    		FileUtils.copyFile(src, new File("src/main/resources/passed-screenshots/" + fileName + ".PNG"));
    ...
    public static void SelectCalendarDateByTitle(WebDriver driver, WebElement TablesBody, String SelectByColumnName) {
        ...
        for (WebElement cell: columns) {
            ...
            if (cell.getAttribute("title").equalsIgnoreCase(SelectByColumnName)) {
            	BaseTest.MoveToElement(driver, cell);
            	BaseTest.Wait(2);
                cell.click();
                BaseTest.Wait(2);
                break;
